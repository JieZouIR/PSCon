...
class T6(nn.Module):
    """
    T6 model for response generation with context, query and passage information
    ...
    def forward(self, context, query, passage, response, common_output):
        ...
        Args:
            ...
            passage: input passage sequence [batch_size, num_passages, passage_len]
        ...
        """
        # print(f"[T6 Forward Start] Response shape: {response.shape}")
    ...
    def loss(self, t6_output, response):
        ...
        Args:
            t6_output: model predictions [batch_size, response_len, vocab_size]
            response: target response [batch_size, response_len]
        Returns:
            Negative log likelihood loss
        """
        t6_loss = F.nll_loss((t6_output.reshape(-1, t6_output.size(-1)) + 1e-8).log(), response[:, 1:].reshape(-1),
                             ignore_index=0)
        return t6_loss
